...
```{r}
cran #sale como si fuera una tabla
glimpse(cran) #en filas, primero sale el nombre, despues sale la clase y por
#ultimo algunos valores
```
# dplyr
...
# Filtrar los registros que cumplan con las condiciones especificadas
cran %>%
...
```{r}
# Suponiendo que 'cran3' es tu data frame y 'size_bytes' 
#es la columna que contiene el tamaño en bytes de la descarga
...
```{r}
cran <- cran %>% mutate(correct_size = size + 1000)
```
...
```{r}
avg_bytes <- cran %>% summarise(mean(size))
```
# Agrupamientos
...
top_counts <- cran %>%
  group_by(package) %>%
  summarize(descargados = n()) %>%
  ungroup() %>%
  mutate(umbral = quantile(descargados, probs = 0.99, na.rm = TRUE)) %>%
  filter(descargados > umbral)
...
library(tidyverse)
#las variables son grade, male, female 
datos_grade_tidy <- students %>%
  pivot_longer(cols = c(male, female),
               names_to = "sex",
               values_to = "count")


head(datos_grade_tidy)
...
# Extraer el día de la semana (en formato numérico)
wday(this_day)
